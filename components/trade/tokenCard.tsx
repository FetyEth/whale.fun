...
const TokenCard = ({ token, index, compact = false }: TokenCardProps) => {
  ...
  return (
    <div
      ...
      className={`relative rounded-2xl ${compact ? "p-4" : "p-5"} shadow-md cursor-pointer transition-transform hover:scale-[1.01] flex flex-col justify-between ${sizeClass} overflow-hidden`}
      onClick={handleCardClick}
    >
      {/* Content on the left */}
      <div className={`flex-1 min-w-0 ${compact ? "pr-[128px]" : "pr-[170px]"} z-10`}>
        <div className={`${compact ? "text-[17px]" : "text-xl"} font-extrabold tracking-tight truncate ${headingClass}`}>${token.symbol.toUpperCase()}</div>
        <div className={`mt-1 ${compact ? "text-xs" : "text-sm/5"} ${textClass}`}>Market Cap</div>
        <div className="mt-1 flex items-baseline gap-2">
          <div className={`${compact ? "text-xl" : "text-2xl"} font-bold truncate ${headingClass}`}>{token.marketCap}</div>
          <div className={`${compact ? "text-xs" : "text-sm"} font-medium ${priceChangeClass}`}>
            {token.priceChange}
          </div>
        </div>
        <button
          className={`mt-2 inline-flex items-center gap-1 ${compact ? "text-xs" : "text-sm"} font-semibold underline underline-offset-4 ${textClass} hover:${headingClass}`}>
          View Token →
        </button>
      </div>
      ...
      <div className="mt-auto z-10">
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-lg ${compact ? "px-3 py-1.5 text-xs" : "px-4 py-2 text-sm"} font-semibold shadow-sm ${theme.quickBuy}`}>
          + Quick buy 0.01
        </button>
      </div>

      {/* Image on the right (vertically centered, fixed 146.2px square) */}
      <div className="absolute right-0 top-1/2 -translate-y-1/2" style={{ height: imgSize, width: imgSize }}>
