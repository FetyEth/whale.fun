...
export default function StudioPage() {
  ...
  const [recordings, setRecordings] = useState<
    Array<{ url: string; size?: number; ts: number }>
  >([]);
  ...
  const [qa, setQa] = useState<
    Array<{ q: string; ts: number; sentiment: number }>
  >([]);
  ...
  const handleCreateAndGo = useCallback(async () => {
    try {
      ...
      setRoomId(newRoomId);
      const link =
        (roomData as any).meetingLink ||
        `https://app.huddle01.com/room/${newRoomId}`;
      setMeetingLink(link);
      ...
      router.push(
        `/studio/room/${encodeURIComponent(
          newRoomId
        ...
        setRecordings((prev) => [
          {
            ...
            size: data?.recordingSize,
            ts: Date.now(),
          },
          ...prev,
        ]);
        if (confirm("Recording stopped. Open recording URL?"))
  ...
  const buyToken = useCallback(async () => {
    try {
      if (!tokenAddress || !buyAmount)
        return alert("Token address and amount required");
  ...
  const requestAVPermission = useCallback(async () => {
    try {
      ...
      alert(
        e?.message || "Failed to get camera/mic. Check browser permissions."
      );
    }
  }, []);
  ...
  const reputationScore = useMemo(() => {
    ...
    try {
      ...
      const totalVolumeGenerated = Number(
        creatorMetrics.totalVolumeGenerated ?? 0
      );
  ...
  const askQuestion = () => {
    ...
    setQa((prev) => [
      { q: question.trim(), ts: Date.now(), sentiment: s },
      ...prev,
    ]);
    setQuestion("");
  };
    ...
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-gray-500">
          Create your room and go live with Whale.fun.
        </p>
      </header>
        ...
        <div className="sticky top-0 z-10 -mt-2 mb-4 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 border rounded p-3 flex flex-wrap items-center gap-2">
          ...
          <div className="w-full md:w-auto md:ml-2 flex items-center gap-2">
            ...
            <button
              ...
              className={`text-xs px-2 py-1 rounded ${
                showQA ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"
              }`}
            >
              Q&A
            </button>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-lg border p-4 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={toggleMic}
                disabled={!joinedPreview || inPageJoin}
            ...
            <p className="text-xs text-gray-500">
              This is a local preview. Use &quot;Open Meeting&quot; to join the
              actual Huddle room; keep this preview to test mic/video before
              going live.
            </p>
          </div>
        </section>
      ...
      {currentStep === 3 && (
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-lg border p-4 space-y-3">
            ...
            <p className="text-xs text-gray-500">
              Recorder bot will join your room and record the session. You can
              also control recording in the live room.
            </p>
            {/* Recording history */}
            {recordings.length > 0 && (
              <div className="mt-3">
                  ...
                  {recordings.map((r, idx) => (
                    ...
                    >
                      <span className="truncate">
                        ...
                        {r.size ? `${r.size} bytes` : "size unknown"}
                      </span>
                      <a
          ...
          <div className="rounded-lg border p-4 space-y-3">
            <h2 className="text-lg font-semibold flex items-center gap-2">
              Livestream{" "}
              {livestreaming ? (
                <span className="text-[10px] bg-rose-100 text-rose-700 px-2 py-0.5 rounded">
                  ON
                </span>
              ) : (
                <span className="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded">
            ...
            <p className="text-xs text-amber-700 mt-2">
              Tip: Start/Stop livestream after the host joins the room in
              Huddle.
            </p>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-lg border p-4 space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <button
                onClick={loadCreatorMetrics}
                className="px-3 py-2 rounded bg-gray-800 text-white"
              >
                Load Creator
              </button>
              <button
                onClick={loadPlatformMetrics}
            ...
            {creatorMetrics && (
              <pre className="bg-gray-50 p-2 rounded text-xs overflow-auto">
                {JSON.stringify(creatorMetrics, null, 2)}
              </pre>
            )}
            {platformMetrics && (
              <pre className="bg-gray-50 p-2 rounded text-xs overflow-auto">
          ...
          <div className="rounded-lg border p-4 space-y-3">
            ...
            {tradeStatus && (
              <p className="text-xs text-gray-600">{tradeStatus}</p>
            )}
            {currentStep === 5 && (
              <div className="flex justify-end">
                <button
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-lg border p-4 space-y-3">
                ...
                {nameOptions.map((n) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded border ${
                      selectedName === n ? "bg-blue-600 text-white" : "bg-white"
                    }`}
                  >
                    {n}
                  </button>
            ...
            <div className="text-xs text-gray-600">
              Selected: {selectedName} ({selectedSymbol}) • Supply: {supply} •
              Creator Fee: {creatorFee} bps
            </div>
          </div>
          ...
          <div className="rounded-lg border p-4 space-y-3">
            ...
            <p className="text-xs text-gray-500">
              Live editable contract snippet (Monaco).
            </p>
            <div
              ref={monacoDivRef}
              style={{ height: 300 }}
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="rounded-lg border p-4 space-y-3">
            ...
            <p className="text-sm text-gray-600">
              Need help during a session? Send a support ping.
            </p>
            <button
              onClick={async () => {
